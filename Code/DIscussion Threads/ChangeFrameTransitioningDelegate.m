...
//  Copyright (c) 2017 Spastic Muffin, LLC. All rights reserved.
//
...
- (void) viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator;
{
    ...
    CGRect frame = _dimmingView.frame;
    frame.size = increasedSize;
...
- (void) recenterThePresentedView;
{
    [self setCenterOfPresentedViewWithSize:[self containerView].frame.size];
}
...
- (void)presentationTransitionWillBegin
{
    ...
    if([presentedViewController transitionCoordinator])
    {
        [[presentedViewController transitionCoordinator] animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
            ...
            NSLog(@"self.presentedViewController.view: %@", self.presentedViewController.view);
        }];
    }
    else
    {
...
{
    NSLog(@"frameOfPresentedViewInContainerView: %@", NSStringFromCGRect(self.currentFrame));
...
- (CGSize) size;
{
    return self.presentedView.bounds.size;
}
...
- (void) close;
{
    [(PresentationController *) self.presentationController close];
}
