...
class ShareAlbum {
    ...
    func start() {
        ...
        if SignInManager.session.userIsSignedIn {
            ...
            alert = UIAlertController(title: "Share\(albumName) album images with a Dropbox, Facebook, or Google user?", message: nil, preferredStyle: .actionSheet)
    ...
    private func completeSharing(permission:Permission) {
        SyncServerUser.session.createSharingInvitation(withPermission: permission, sharingGroupUUID: sharingGroup.sharingGroupUUID) {[unowned self] invitationCode, error in
            if error == nil {
