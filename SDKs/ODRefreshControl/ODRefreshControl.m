...
{
    self = [super initWithFrame:CGRectMake(0, -(kTotalViewHeight + scrollView.contentInset.top + self.yOffset), scrollView.frame.size.width, kTotalViewHeight)];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"contentInset"]) {
        if (!_ignoreInset) {
            ...
            self.frame = CGRectMake(0, -(kTotalViewHeight + self.scrollView.contentInset.top + self.yOffset), self.scrollView.frame.size.width, kTotalViewHeight);
        }
        return;
    }
